  ...
  <div class="box-body table-responsive no-padding">
    ...
    <table class="table table-bordered" id="view">
      <thead>
        ...
        <tr>
          <td>{{getFormatedDate($schedule->schedule_date)}}</td>
          <td>{{$schedule->beat_name}}</td>
          <td>{{$schedule->contact_name}}</td>
          <td>{{$schedule->contact_number}}</td>
          <td>{{$schedule->address}}</td>
          <td> 
          <a class="td-action-btn point-this" data-toggle="tooltip" data-placement="top" title="Test" target="_blank" href="/fapp/{{$schedule->my_schedule_id}}" >
